...
import { deletelunch, getLunchDetails } from '../actions/lunchActions'
import { Link } from 'react-router-dom'
import { CREATE_LUNCH_RESET, DELETE_LUNCH_RESET, UPDATE_LUNCH_RESET, MPESA_STK_PUSH_REQUEST } from '../constants'
...
const LunchDetailsPage = () => {
  ...
  // const lunchId = searchParams.get('id')
  const {id} = useParams()
  ...
  const deleteLunchReducer = useSelector(state =>state.deleteLunchReducer);
  const { success: lunchDeletionSuccess} = deleteLunchReducer
  ...
  useEffect(()=>{
    ...
    dispatch({
      type: UPDATE_LUNCH_RESET
    });
    dispatch({
      type: CREATE_LUNCH_RESET
    });
    dispatch({
  ...
  const confirmDelete = () => {
    dispatch(deletelunch(id));
    handleClose()
  }

  if (lunchDeletionSuccess) {
    alert("Lunch Menu successfully deleted.");
    router('/');
    dispatch({
      type: DELETE_LUNCH_RESET
    })
            ...
            {error ? <Error variant='danger'>{error}</Error> :
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                    <div className='max-w-md bg-gray-800 border border-gray-700 rounded-lg shadow'>
                        <img className='rounded-t-lg w-full' src={fullImageUrl} alt={lunch.name} />
                        {userInfo && userInfo.admin &&
                            <div className='flex justify-between p-4'>
                                ...
                                <button className='bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded' onClick={() => router(`/product-update/${lunch.id}/`)}>Edit Product</button>
                            </div>
                        }
                    </div>
                    <div>
                        <p className='font-semibold text-3xl text-gray-400'>{lunch.name}</p>
                        <hr className='my-8 bg-gray-700 rounded'></hr>
                        {/* <p className='text-gray-500 text-lg'>{lunch.description}</p> */}
                        <p className='text-lg text-gray-200 mt-10'>Price: <span className="text-accent ml-2">Ksh {lunch.price}</span></p>
                    </div>
                    <div className='mb-5'>
                    ...
                    <div className="modal-content py-4 text-left px-6">
                        ...
                        <div className="modal-body py-4 text-gray-500">
                            <p className="text-base leading-relaxed">
                                Are you sure you want to delete this product <span className="font-bold">{lunch.name}</span>?
                            </p>
                        </div>
...
export default LunchDetailsPage